...
def kandala_circuit(param, wires):
    qml.BasisState(np.zeros(len(wires)), wires=wires)
    for i in range(len(param)):
        qml.RZ(param[i], wires='q1')
...
def VQE(th0,cost_fc=cost_fn, gradDetect=False):
    """
    ...
    if gradDetect==True:
        plt.plot(np.linspace(0, n, n+1), grad,  marker='X', linestyle='dashed', label=r'$s=$'+str(s))
        plt.legend()
...
for sind, s in enumerate(sarray):
    ...
    print(s)
    n, theta, E, t=VQE(th0,cost_fc=cost_fnAA, gradDetect=True)
    senergy.append(E)
    sangle.append(theta)
    sn.append(n)
    ts.append(t)
